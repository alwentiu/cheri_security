...
void scan_recursive(void* cap, seenCapabilities* seenHead, int print_cap){
    ...
    for(int i = 0; i < len; i+=sizeof(void*)){
        ...
        if(!cheri_is_sealed(new_cap) && cheri_tag_get(new_cap) && (perms & CHERI_PERM_LOAD_CAP) == CHERI_PERM_LOAD_CAP && ((perms & PERM_SEAL) != PERM_SEAL)){
            //printf("Read perm: %p: %#p\n", (cap+i), new_cap);

            if(!isInList(new_cap, seenHead)){
                printf(" - new cap, recursing: %#p, %s\n", new_cap, cap_to_str(NULL, new_cap));
...
static void malware()
{
    ...
    printf("starting recursive scan on stack\n");
